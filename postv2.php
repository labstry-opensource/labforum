  ...
  if(@$_GET['action']=='submit' && !@$_GET['mode']){
    ...
    if($t_name && $content){
      if(strlen($t_name) <= 70){
        ...
        mysqli_query($connect, "INSERT INTO threads(fid, topic_name, topic_content, topic_creator, draft, date, rights, seo) VALUES ('$fid', '".$t_name."','".$content."','".@$_SESSION['username']."','".$draft."', NOW(), '".$readpermission."' , '".$seo."');");
