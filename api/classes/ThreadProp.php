...
class ThreadProp{
	...
	public function getThreadProp(){
		$stmt = $this->pdoconnect->prepare("SELECT * FROM threads WHERE topic_id = ?");
	...
	*/
	...
	public function getThreadProp(){
		$stmt = $this->pdoconnect->prepare("SELECT 
			reply_topic, reply_content, author, u.username, r.date, hiddeni, profile_pic
			FROM `php_forum`.replies r, `userspace`.`users` u 
			WHERE topic_id = ? AND reply_id = ? AND u.id = r.author");
		...
		if($threadarr["hiddeni"] == 1){
			$threadarr["reply_topic"] = "帖子已屏蔽";
			$threadarr["reply_content"] = "本帖子因內容不恰當而被屏蔽,如有疑問請聯絡管理員";
		}
