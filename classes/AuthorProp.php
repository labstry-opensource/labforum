...
class AuthorProp{
	...
	public function checkUserBySpecialTeam(){
		$stmt = $this->pdoconnect->prepare("SELECT 
			...
			`php_forum`.`roles`.rights
			 ...
			 `userspace`.`users`.`username` = `php_forum`.`specialteam`.`username` AND 
			 `userspace`.`users`.`id` = `php_forum`.`threads`.`author` AND 
	...
	public function checkUserByNormalUser(){
		...
		$stmt = $this->pdoconnect->prepare("SELECT 
			...
			WHERE
			`php_forum`.`threads`.`author` = `userspace`.`users`.`id` AND
			`php_forum`.`threads`.`topic_id` = ?");
