...
<div style="width:100%;position:absolute;top:150px;display: none;padding-left: 50px;" class="searchresultprovider">
	<div class='row'>閱讀權限:<input style='display:inline-block;' class='settingsblank permissionshow' name='permission' value='0'></input>
	<div id='readhint' style='display:none;background-color:red;color:white;margin-left:15px'>權限必須介乎0-255 之間</div>
	</div>
	<div class='row' style='display:inline-block'>草稿模式:<a href='#' class='toggle draft'>Off</a></div>
	<div style='display:inline-block'>該功能暫未啟用</div>
</div>
<?php
    if((@$_GET['mode'] == 'edit' && @$_GET['id']) || !@$_GET['mode'] == 'edit'){
      ...
      echo "<form action='";
      if(@$_GET['mode'] == 'edit') echo "postv2.php?id=".$editid."&mode=edit&action=submit";
      else echo "postv2.php?action=submit";
...
<?php
	   ...
	   echo "<input name='readpermission' class='readpermission' style='display:none' value='0'/>";  
	   echo "</div></div>";
...
<script>
  ...
  function submit_form(){
	  var myform = document.getElementById('postedit');
    $('#content').val($('#formatfaker').contents().find("body").html());
	  alert($('#content').val());
	  //$('#postedit').submit();
  };
  function changeFont(){
  ...
  if(@$_GET['action']=='submit' && !@$_GET['mode']){
    ...
    if($t_name && $content){
      if(strlen($t_name) <= 70){
        ...
        mysqli_query($connect, "INSERT INTO threads(fid, topic_name, topic_content, topic_creator, date, rights) VALUES ('$fid', '".$t_name."','".$content."','".@$_SESSION['username']."', NOW(), '".$readpermission."');");
          mysqli_query($connect, "UPDATE users SET score=score+2 , topics=topics+1 WHERE username='".@$_SESSION['username']."'");
          $check= mysqli_query($connect, "SELECT topic_id FROM threads WHERE date ='$currtime' 
                       AND topic_creator='".@$_SESSION['username']."';");
  ...
  if(@$_GET['action'] == 'submit' && @$_GET['mode']=='edit'){
    if($t_name && $content){
      if(strlen($t_name) <=70){
        $query = mysqli_query($connect, "UPDATE threads SET topic_name = '".$t_name."', topic_content = '".$content."', rights = '".$readpermission."'
          WHERE topic_id = ".$editid.";");
        echo "<script>window.location = 'thread.php?id=$tid'</script>";
      } else echo $wordcount;
